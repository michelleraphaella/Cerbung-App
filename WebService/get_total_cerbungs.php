<?php
	...
	$sql = "SELECT idcerbung, title, date_format(substr(tanggal, 1, 10), '%d/%m/%Y') as tanggal, jumlah_paragraf, jumlah_like, description, access, url, username, idgenre FROM cerbung WHERE username=?";
	$stmt = $c->prepare($sql);
